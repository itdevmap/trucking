	...
	<script>
		...
		function ReadData(hal) {
			...
			$.get("ajax/part_crud.php", {
				...
				cari:cari,
				hal:hal,
				type:"Read_GR" }, function (data, status) {
			...
			if (confirm("Are you sure to Execute ?")) {
				$.ajax({
					...
					error: function (xhr, status, error) {
						...
						alert("Terjadi kesalahan saat eksekusi. Periksa console log.");
					}
				});
			}
		}

		function SendGR(id_part_gr, el) {
			var $btn = $(el);

			// Ubah tampilan tombol jadi "Proses..."
			$btn.text("Proses...").css({
				pointerEvents: "none",
				opacity: 0.6
			});
			...
			$.ajax({
				url: "ajax/part_crud.php",
				type: "POST",
				data: { type: "SaveUpGR", id_part_gr: id_part_gr },
				dataType: "json",
				success: function (res) {
					if (res.success === false) {
						alert("Gagal: " + res.message);
						// kembalikan tombol ke posisi awal
						$btn.text("Send GR").css({
							pointerEvents: "auto",
							opacity: 1
						});
					} else {
						...
						console.log(res);
						// ubah jadi "Selesai" dan tetap disable
						$btn.text("Selesai ✅").css({
							pointerEvents: "none",
							opacity: 0.7,
							color: "green"
						});
				...
				error: function (xhr, status, err) {
					...
					// jika error, tombol dikembalikan
					$btn.text("Send GR").css({
						pointerEvents: "auto",
						opacity: 1
					});
					...
					<div style="width:99%;border-bottom:2px solid #83a939;background:none;margin-left:-5px;margin-top:-5px;margin-bottom:-9px" class="input-group">	
						...
						<div id="tabs5" >
							<ul> 
								...
								<li id="current"><?php echo "<a href=$link4>"; ?><span><b>Data In (GR)</b></span></a></li>
							</ul>
						</div>	
					</div>					
				</div>	
							...
							<span class="input-group-addon" style="width:50%;text-align:left;padding:0px">
								<?php if ($m_add == '1'){
									...
									<button class="btn btn-block btn-success"
										style="margin:0px; margin-left:0px; margin-bottom:0px; border-radius:2px;"
										type="button"
										title="Created Order"
										onclick="window.location.href='part_gr_data.php?mode=Add'">
					...
					<div class="col-md-12" style="min-height:40px;border:0px solid #ddd;padding:0px;border-radius:5px;">
						<div class="box box-success box-solid" style="padding:5px;border:1px solid #ccc">	
							...
							<div style="width:100%;" class="input-group">
								...
								<textarea id="remark" style="text-align:left; width:80%; border:1px solid rgb(169, 169, 169); resize:yes;"></textarea>	
