...
$pq = mysqli_query($koneksi, "select * from m_role_akses where id_role = '$id_role'  and id_menu ='9' ");
...
if ($_GET['type'] == "Read")
{
	...
	$data = '<table class="table table-hover table-striped" style="width:100%">
			<thead style="font-weight:500px !important">
				<tr>					
					...
					<th rowspan="2" width="76%" style="text-align: center;">PORT NAME</th>
					<th rowspan="2" width="5%" style="text-align: center;">CODE</th>
	...
	$SQL = "select * from m_port where nama_port LIKE '%$cari%' order by nama_port  LIMIT $offset, $jmlperhalaman";	
    ...
    if(mysqli_num_rows($result) > 0)
    {
    	...
    	{	
			$posisi++;		
				...
				<td style="text-align:left">'.$row['nama_port'].'</td>
				...
				if($m_edit == '1'  ){
					$data .= '<td>
								<button class="btn btn-block btn-default" title="Edit"
									...
									onClick="javascript:GetData('.$row['id_port'].')"  >
	...
	$data .= '<div class="paginate paginate-dark wrapper">
				<ul>';
				$pq = mysqli_query($koneksi, "select count(*) as jml from m_port where nama_port LIKE '%$cari%' ");					
				$rq=mysqli_fetch_array($pq);
				$total_record = $rq['jml'];										
	...
	{	
		$id = $_POST['id'];
		$nama_port = trim(addslashes(trim(strtoupper($_POST['nama_port']))));
		$caption = addslashes(trim(strtoupper($_POST['caption'])));
		$stat = $_POST['stat'];
		$mode = $_POST['mode'];
		if($mode == 'Add')
		{			
			$sql = "INSERT INTO m_port (nama_port, caption,created,status) values
					('$nama_port','$caption','$id_user','$stat')";
			$hasil=mysqli_query($koneksi, $sql);
		}
		else
		{
			$sql = "update m_port set 
					nama_port = '$nama_port',
					caption = '$caption',
					status = '$stat'
					where id_port = '$id'	";
	...
	$id = $_POST['id'];	
    $query = "select * from m_port where id_port  = '$id'";
...
{	
	...
	$data = '<table class="table table-hover table-striped" style="width:100%">
			<thead style="font-weight:500px !important">
				<tr>
					...
					<th width="87%" style="text-align: center;">PORT NAME</th>
	...
	$SQL = "select * from m_port where nama_port LIKE '%$cari%' and status = '1'  order by nama_port LIMIT 0, 25";
    ...
    if(mysqli_num_rows($result) > 0)
    {
    	...
    	{	
			...
			$data .= '<td style="text-align:left"><a href="#" onclick="PilihPort('.$row['id_port'].')" >'.$row['nama_port'].'</a></td>';
			$data .= '<td style="text-align:center">
						<button type="button" class="btn btn-default" onClick="javascript:PilihPort('.$row['id_port'].')" 
