...
if ($_GET['type'] == "Read")
{
	...
	$data = '<table class="table table-hover table-striped" style="width:100%">
			<thead style="font-weight:500px !important">
				<tr>					
					...
					<th rowspan="2" width="80%" style="text-align: center;">NAMA SALES</th>
	...
	$SQL = "select * from  m_sales_tr where nama LIKE '%$cari%' order by nama  LIMIT $offset, $jmlperhalaman";	
				...
				if($m_edit == '1' ){
					$data .= '<td>
								<button class="btn btn-block btn-default" title="Edit"
									...
									onClick="javascript:GetData('.$row['id_sales'].')"  >
	...
	$data .= '<div class="paginate paginate-dark wrapper">
				<ul>';
				$pq = mysqli_query($koneksi, "select count(*) as jml from  m_sales_tr where nama LIKE '%$cari%' ");					
				$rq=mysqli_fetch_array($pq);
				$total_record = $rq['jml'];										
	...
	{	
		...
		if($mode == 'Add')
		{			
			$sql = "INSERT INTO  m_sales_tr (nama,created,status) values
					('$nama','$id_user','$stat')";
			$hasil=mysqli_query($koneksi, $sql);
		}
		else
		{
			$sql = "update  m_sales_tr set 
					nama = '$nama',
					status = '$stat',
					created = '$id_user'
					where id_sales = '$id'	";
	...
	$id = $_POST['id'];	
    $query = "select * from  m_sales_tr where id_sales  = '$id'";
...
{	
	...
	$SQL = "select * from  m_sales_tr where nama LIKE '%$cari%' and status = '1'  order by nama LIMIT 0, 25";
    ...
    if(mysqli_num_rows($result) > 0)
    {
    	...
    	{	
			...
			$data .= '<td style="text-align:left"><a href="#" onclick="PilihCost('.$row['id_sales'].')" >'.$row['nama'].'</a></td>';
			$data .= '<td style="text-align:center">
					<button type="button" class="btn btn-default" onClick="javascript:PilihCost('.$row['id_sales'].')" 
