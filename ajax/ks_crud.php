...
if ($_GET['type'] == "Read")
{
	...
	$data = '<table class="table table-hover table-striped" style="width:100%">
			<thead style="font-weight:500px !important">
				<tr>					
					...
					<th rowspan="2" width="7%" style="text-align: center;">KOMISI</th>
	...
	if($stat == 'All')
	{
		...
		$SQL = "select t_jo_mobil.*, m_kota.nama_kota as asal, m_kota1.nama_kota as tujuan, m_mobil.no_polisi,
			   ...
			   left join m_supir on t_jo_mobil.id_supir = m_supir.id_supir
			   where t_jo_mobil.jenis = '0' and t_jo_mobil.komisi > 0 and 
			   t_jo_mobil.tgl_sj between '$tgl1x' and '$tgl2x' and $f LIKE '%$cari%' and t_jo_mobil.komisi_status = '$stat' 
				  order by t_jo_mobil.tgl_sj desc
			  LIMIT $offset, $jmlperhalaman";
	}
    ...
    if(mysqli_num_rows($result) > 0)
    {
    	...
    	{	
			...
			$sisa = $row['komisi'] - $row['komisi_bayar'];
			$tagihan = number_format($row['komisi'],0);
			$bayar = number_format($row['komisi_bayar'],0);
			$sisax = number_format($sisa,0);
			
			$t_tagihan = $t_tagihan + $row['komisi'];
			$t_bayar = $t_bayar + $row['komisi_bayar'];
			...
			$update = mysqli_query($koneksi, "update t_jo_mobil set komisi_status = '$s' where id_detil = '$row[id_detil]'  ");
	...
	{		
		...
		$sql = "INSERT INTO t_komisi_bayar (id_jo_mobil, id_jurnal, bayar, pph, id_bank) 
		    values ('$id_sj','$id_jurnal','$jumlah_bayar','$pph','$id_bank')";
		$hasil=mysqli_query($koneksi, $sql);
	
		$bayar = $jumlah_bayar + $pph;
		$sql = "UPDATE t_jo_mobil set komisi_bayar =  komisi_bayar + '$bayar' where id_detil = '$id_sj' ";
		$hasil = mysqli_query($koneksi, $sql);
...
}else if($_GET['type'] == "ListPayment")
{
	...
	$t1="select t_komisi_bayar.*, t_jurnal.id_user, t_jurnal.jumlah, t_jurnal.tgl_jurnal, t_jurnal.no_jurnal,m_bank.nama_bank, m_bank.no_bank from
	        t_komisi_bayar inner join t_jurnal on t_komisi_bayar.id_jurnal = t_jurnal.id_jurnal
			left join m_bank on t_komisi_bayar.id_bank = m_bank.id_bank
			where t_komisi_bayar.id_jo_mobil = '$id' order by t_jurnal.tgl_jurnal";
	...
	$rq=mysqli_fetch_array($pq);	
	$tagihanx = number_format($rq['komisi'],0);
	$sisa = $rq['komisi'] - $total;
	$data .= '<tr>							
			<td colspan = "4" style="text-align:center;background:#eaebec"></td>	
			<td colspan= "2" style="text-align:right;background:#e48f0f;color:#fff">Komisi</td> 
	...
	$id = $_POST['id']; 
	$pq = mysqli_query($koneksi, "select * from t_komisi_bayar where id_bayar = '$id'  ");
	$rq=mysqli_fetch_array($pq);
	$id_jurnal = $rq['id_jurnal'];
	Del_Jurnal($id_jurnal);
	$jumlah = $rq['bayar'] + $rq['pph'];	
	
	$update = mysqli_query($koneksi, "UPDATE t_jo_mobil set komisi_bayar =  komisi_bayar - '$jumlah'  where id_detil = '$rq[id_jo_mobil]' ");
    $query = "DELETE FROM  t_komisi_bayar WHERE id_bayar = '$id' ";
    if (!$result = mysqli_query($koneksi, $query)) {
        exit(mysql_error());
    }		
