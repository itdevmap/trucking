...
$pq = mysqli_query($koneksi, "select * from m_role_akses_tr where id_role = '$id_role'  and id_menu ='20' ");
...
if ($_GET['type'] == "Read")
{
    ...
    if(mysqli_num_rows($result) > 0)
    {
    	...
    	{	
			$posisi++;		
				...
				<td style="text-align:left">'.$row['nama_kota'].'</td>
				...
				if($m_edit == '1'  ){
					$data .= '<td>
								<button class="btn btn-block btn-default" title="Edit"
									...
									onClick="javascript:GetData('.$row['id_kota'].')"  >
	...
	$data .= '<div class="paginate paginate-dark wrapper">
				<ul>';
				$pq = mysqli_query($koneksi, "select count(*) as jml from m_kota_tr where nama_kota LIKE '%$cari%' ");					
				$rq=mysqli_fetch_array($pq);
				$total_record = $rq['jml'];										
	...
	{	
		...
		if($mode == 'Add')
		{			
			$sql = "INSERT INTO m_kota_tr (nama_kota,created,status) values
					('$nama','$id_user','$stat')";
			$hasil=mysqli_query($koneksi, $sql);
		}
		else
		{
			$sql = "update m_kota_tr set 
					nama_kota = '$nama',
					status = '$stat',
					created = '$id_user'
					where id_kota = '$id'	";
