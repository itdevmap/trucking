...
if ($_GET['type'] == "Read")
{
	...
	$data = '<table class="table table-hover table-striped" style="width:100%">
			<thead style="font-weight:500px !important">
				<tr>					
					...
					<th rowspan="2" width="80%" style="text-align: center;">JENIS BAN</th>
	...
	$SQL = "select * from m_jenis_ban where nama LIKE '%$cari%' order by nama  LIMIT $offset, $jmlperhalaman";	
				...
				if($m_edit == '1'  ){
					$data .= '<td>
								<button class="btn btn-block btn-default" title="Edit"
									...
									onClick="javascript:GetData('.$row['id_jenis_ban'].')"  >
	...
	$data .= '<div class="paginate paginate-dark wrapper">
				<ul>';
				$pq = mysqli_query($koneksi, "select count(*) as jml from m_jenis_ban where nama LIKE '%$cari%' ");					
				$rq=mysqli_fetch_array($pq);
				$total_record = $rq['jml'];										
	...
	{	
		...
		if($mode == 'Add')
		{			
			$sql = "INSERT INTO m_jenis_ban (nama,status,created) values
					('$nama','$stat','$id_user')";
			$hasil=mysqli_query($koneksi, $sql);
		}
		else
		{
			$sql = "update m_jenis_ban set 
					nama = '$nama',
					status = '$stat',
					created = '$id_user'
					where id_jenis_ban = '$id'	";
			$hasil=mysqli_query($koneksi, $sql);
		}
		if (!$hasil) {
	...
	$id = $_POST['id'];	
    $query = "select * from m_jenis_ban where id_jenis_ban  = '$id'";
